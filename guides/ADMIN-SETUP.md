...
Repeter la procedure suivante pour chacun des trois comptes.
...
3. Lancer le script d'installation :
   ...
   **Linux / macOS**
   ...
   Le script installe le CLI `sooacel` et le rend disponible dans le PATH.
...
> **Note :** Le CLI `sooacel` remplace les anciens aliases `ve-*`.
...
4. Relancer `install.sh` / `install.ps1` apres avoir mis a jour le script d'installation
   pour que le nouveau compte soit pris en charge par le CLI `sooacel`.
...
- Instruire les developpeurs de **n'utiliser le CLI `sooacel` que pour les operations
